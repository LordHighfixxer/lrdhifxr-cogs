    ...
    async def _score(self, context, member: discord.Member):
        ...
        if server.id in self.scores:
            if member.id in self.scores[server.id]:
                message = '**{} shot a total of {} minions of the darkness ({})**'.format(member.mention, self.scores[server.id][member.id]['total'], ', '.join([str(p[hunter]['score'][x]) + ' ' + x.capitalize() for x in p[hunter]['score']])
            else:
