...
# I wanted to preserve as much compatibility as possible so CrewPoll
...
class CrewPoll:
    ...
    async def cpoll(self, ctx, *text):
        """Starts/stops a reaction poll
        Usage example (time argument is optional)
        cpoll Is this a poll?;Yes;No;Maybe;t=60
        cpoll stop"""
        message = ctx.message
        ...
        if not self.getPollByChannel(message):
            ...
            if not channel.permissions_for(server.me).manage_messages:
                await self.bot.say("I require the 'Manage Messages' "
                                   ...
                                   "a reaction poll.")
                return
            p = NewCrewPoll(message, " ".join(text), self)
            if p.valid:
                self.poll_sessions.append(p)
                await p.start()
            else:
                await self.bot.say("`[p]cpoll question;option1;option2...;t=60`")
        else:
