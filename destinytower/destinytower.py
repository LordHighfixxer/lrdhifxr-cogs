...
class quotepa:
    ...
    @commands.command(no_pm=True, aliases=["quotepa"])
    async def quotepa(self):
        """Displays a random quotepa."""

        # TODO make bigger list
        quotepa = ["Attention Guardians, attacks on Titans have increased. Please stay in a fireteam to remain safe.",
               ...
               "Shuttle Advisory: The Crucible is open to those who run it.",!kesse
        ...
        quotepa = choice(quotepa)

        quotepasay = ["quotepa!!11", "quotepa Intensification Protocol Engaged", "Keeeeeeesssssssel",
                  "quotepa quotepa :P", "*intense quotepaing*", "I ran out of quotepa so here is some more"]
        quotepasay = choice(quotepasay)
        ...
        data.add_field(name=quotepasay, value=u"\u2063")
        data.set_image(url=quotepa)
