...
class The100:
        ...
        for s, x in enumerate(response, start=1):
            ...
            for y in ["category", "team_size", "time_zone",
                ...
                if y == "confirmed_sessions":
                    ...
                    msg += "Start time (all times Pacific): " + " ".join((time[:-9] + " " + time[-5:]).split("T")) + "\n"
    ...
    async def token(self, ctx, token: str, *, sname: str=None):
        """Allows you to set the API Token for retrieving information.
           ...
           If on PM, add the server name complete with spaces after the token. No need for quotations"""
        server = ctx.message.server
        if isinstance(ctx.message.channel, discord.PrivateChannel) and sname is not None:
            sname = [server for server in self.bot.servers if sname.lower() == server.name.lower()]
            if ctx.message.author.id == sname[0].owner_id:
                if sname[0].id not in self.db:
                    self.db[sname[0].id] = {"token": token, "role": None}
                else:
                    server = sname[0]
                    if self.db[server.id]["token"]:
                ...
                self.bot.say("No permission to set key.")
                return
        elif isinstance(ctx.message.channel, discord.PrivateChannel) and sname is None:
            await self.bot.say("Put the server name with the correct spacing after the token, thanks.")
            return
        elif not self.permcheck(ctx):
            return
        else:
            if server.id not in self.db:
                self.db[server.id] = {"token": token, "role": None}
            else:
                if self.db[server.id]["token"]:
                    await self.bot.say("Are you sure you want to overwrite the current token? Yes/No")
                    answer = await self.bot.wait_for_message(timeout=15,
    ...
    async def role(self, ctx, *, role: str):
        ...
        server = ctx.message.server
        if server.id not in self.db:
            self.db[server.id] = {"token": None, "role": role}
        else:
