...
class Russianroulette:
                    ...
                    if self.enough_points(user, bet):
                        if not self.rrgame["System"]["Roulette Initial"]:
                            ...
                            await self.bot.say(user.name + " has started a game of roulette, with a starting bet of " +
                                               str(bet) + ".\n" "The game will start in 90 seconds or until I get 5 more players."
                                               " Its time to unbunch your panties and step up like a man " + discord.utils.get(ctx.message.server.roles, name="Risk Takers").mention)
    ...
    async def roulette_game(self, server):
        ...
        if high_noon >= 3:
            ...
            v = ", ".join(noon_names)
            boom = " **BOOM!** " * i
            await self.bot.say("Eddie appears!")
            await asyncio.sleep(1)
            await self.bot.say("It's high noon mother fuckers...")
            await asyncio.sleep(3)
            await self.bot.say(str(boom))
            await asyncio.sleep(1)
            await self.bot.say("```" + str(v) + " just bit the dust." + "```")
            await asyncio.sleep(2)
            await self.bot.say("That's right, this pile of credits is mine.  Time to go see Sandy.")
            self.system_reset()
            await asyncio.sleep(2)
            await self.bot.say("Game Over Losers")
    ...
    async def roulette_round(self, count, player_names, turn):
        ...
        while furd == 0:
            ...
            if chance > 1:
                ...
                await self.bot.say(str(name) + " slowly squeezes the trigger...")
                await asyncio.sleep(4)
                await self.bot.say("**BOOM!** http://i.imgur.com/57N9AlB.gifv")
                await asyncio.sleep(1)
